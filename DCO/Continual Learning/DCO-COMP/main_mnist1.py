...
from models1 import MLP, Conv, InvAuto
...
from algorithm import train_invauto, train_comauto, evaluate_invauto, create_invauto

def evaluate_current_task_loss(model, data_loader):
    model.eval()
    total_loss = 0
    with torch.no_grad():
...
def main():
    ...
    for epoch in range(1, args.lr_epochs+1):
        ...
        if epoch % log_interval == 0:
            ...
            ####
            if epoch==args.lr_epochs:
                average_error=sum(errors)/len(errors)
                if average_error<70:
                    print("Criteria met, modifying the model architechture")
                    mod_main.add_layers(num_layers=2, hidden_size=400)
                    optimizer=torch.optim.Adam(mod_main.parameters(),lr=args.main_online_lr)
    ...
    for m_task in range(2, args.num_tasks+1):
        if args.cl_method == 'sgd':
            ...
            mod_aes += [mod_ae]
            opt_aes += [opt_ae]
            mod_cm, mod_sgs = train_comauto(args, mod_main, mod_aes)
            for mod_ae, mod_sg in zip(mod_aes[1:], mod_sgs):
                mod_ae.layers_E['M'], mod_ae.layers_D['M'] = mod_sg.layers_E, mod_sg.layers_D
            mod_aes[0].layers_E['M'], mod_aes[0].layers_D['M'] = mod_cm.layers_E, mod_cm.layers_D
            opt_aes[0] = torch.optim.Adam([p for n, p in mod_aes[0].named_parameters() if not 'scalars' in n], lr = args.ae_offline_lr, weight_decay=args.ae_weight_decay)
            ...
            for num, (data, target) in enumerate(tr_loaders[m_task]):
                ...
                if args.cl_method == 'sgd':
                    ...
                    for i in range(1, m_task):
                        ...
                        ae_loss += [trainer.ae_reg(args, mod_main, mod_main_centers[i], cl_opt_main, mod_aes[0], opt_aes[0], data, target)]
    ...
    torch.save({args.cl_method: result_list}, 'res-%s-%s-%d-%d.pt'%(args.cl_dataset, args.cl_method, args.cm_topk, args.rank))
    """
