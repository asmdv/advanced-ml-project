...
class MLP(nn.Module):
    ...
    def add_layers(self, num_layers=2, hidden_size=400):
        """
        Dynamically adds additional layers to the MLP model.
        """
        # Extract the existing last layer (output layer)
        output_layer = self.layers[-1]
        # Remove the output layer from the current list
