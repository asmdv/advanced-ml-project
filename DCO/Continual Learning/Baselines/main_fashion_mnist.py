...
def main():
    ...
    if 'fashion' in args.cl_dataset and 'mnist' in args.cl_dataset:
    ...
    if dataset_name == 'to_fashion_mnist':
        tr_loaders += [get_data_loader(get_dataset('mnist', 1, True),  args.train_batch_size, cuda=('cuda' in args.device))]
        te_loaders += [get_data_loader(get_dataset('mnist', 1, False), args.train_batch_size, cuda=('cuda' in args.device))]
        tr_loaders += [get_data_loader(get_dataset('fashion_mnist', 2, True),  args.train_batch_size, cuda=('cuda' in args.device))]
        te_loaders += [get_data_loader(get_dataset('fashion_mnist', 2, False), args.train_batch_size, cuda=('cuda' in args.device))]
    elif dataset_name == 'from_fashion_mnist':
        tr_loaders += [get_data_loader(get_dataset('fashion_mnist', 1, True),  args.train_batch_size, cuda=('cuda' in args.device))]
        te_loaders += [get_data_loader(get_dataset('fashion_mnist', 1, False), args.train_batch_size, cuda=('cuda' in args.device))]
        tr_loaders += [get_data_loader(get_dataset('mnist', 2, True),  args.train_batch_size, cuda=('cuda' in args.device))]
        te_loaders += [get_data_loader(get_dataset('mnist', 2, False), args.train_batch_size, cuda=('cuda' in args.device))]
    else:
        raise ValueError('No matched dataset')
    ...
    for m_task in range(2, args.num_tasks+1):
        if args.cl_method == 'sgd':
            ...
            if dataset_name == 'permuted_mnist':
                ...
                ewc_loader = get_data_loader(get_dataset('fashion_mnist', m_task-1, True), batch_size=1, cuda=False)
            elif dataset_name == 'to_fashion_mnist':
                ewc_loader = get_data_loader(get_dataset('mnist', m_task-1, True), batch_size=1, cuda=False)
            else:
                raise ValueError
            ewc_grads = []
            ...
            if dataset_name == 'permuted_mnist':
                ...
                agem_loader = get_data_loader(get_dataset('fashion_mnist', m_task-1, True), batch_size=1, cuda=False)
            elif dataset_name == 'to_fashion_mnist':
                agem_loader = get_data_loader(get_dataset('mnist', m_task-1, True), batch_size=1, cuda=False)  
            else:
                raise ValueError
        ...
        for cl_epoch in range(args.cl_epochs):
            for num, (data, target) in enumerate(tr_loaders[m_task]):
                ...
                if args.cl_method == 'sgd':
                    ...
                    main_loss.backward()
                    opt_main.step()
                    # pull to the center variable
                    for i in range(1, m_task): 
                        mod_main.module.pull2point(mod_main_centers[i], pull_strength=args.l2_regularization)
                else:
                    raise ValueError('No named method')
