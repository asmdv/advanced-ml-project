        ...
        for epoch in range(1, args.lr_epochs+1):
            ...
            if epoch % log_interval == 0:
                ...
                for i in range(1, args.num_tasks+1):
                    ...
                    errors += [cur_error]
                    visdom_obj.line([cur_error],  [global_epoch + epoch], update='append', opts={'title':'%d-Task Error'%i}, win='cur_error_%d'%i, name = 'T', env='gpu:%d'%args.rank)
                    # Checking only task 1
                    if epoch % args.lr_epochs == 0 and i == 1 and cur_error > cl_error_threshold:
                        print("Current error is bigger than threshold. Adding the new layer and stopping the training.")
        ...
        for m_task in range(2, args.num_tasks+1):
            ...
            for cl_epoch in range(args.cl_epochs):
                ...
                if cl_epoch % log_interval == 0:
                    errors = []
                    for i in range(1, args.num_tasks+1):
                        cur_error = trainer.test(args, mod_main, te_loaders[i], i, global_epoch + (m_task-1)* args.cl_epochs+cl_epoch)
                        errors += [cur_error]
                        visdom_obj.line([cur_error],  [global_epoch + (m_task-1)* args.cl_epochs+cl_epoch], update='append', opts={'title':'%d-Task Error'%i}, win='cur_error_%d'%i, name = 'T', env='gpu:%d'%args.rank)


                        if cl_epoch == (args.cl_epochs - 1) and i <= m_task and cur_error > cl_error_threshold:
                            print("Current error is bigger than threshold. Adding the new layer and stopping the training.")
                            adding_new_hidden_layer = True
                            break
                        elif cl_epoch == (args.cl_epochs - 1) and i <= m_task:
                            print(f"Success. Task {i} Error: {cur_error:.2f}. No need for adding layer")

                    if args.cl_method == 'dco':
                        for i in range(m_task-1):
                            visdom_obj.line([ae_loss[i].item()], [global_epoch + (m_task-1)* args.cl_epochs+cl_epoch], update='append', opts={'title':'%d-AE Loss'%i}, win='ae_loss_%d'%i, name = 'T', env='gpu:%d'%args.rank)
                        print('The grad norm is', grad_norm)
                        try:
                            visdom_obj.line([grad_norm],  [global_epoch + (m_task-1)* args.cl_epochs+cl_epoch], update='append', opts={'title':'Grad Norm'}, win='grad_norm', name = 'T', env='gpu:%d'%args.rank)
                        except:
                            visdom_obj.line([grad_norm.item()],  [global_epoch + (m_task-1)* args.cl_epochs+cl_epoch], update='append', opts={'title':'Grad Norm'}, win='grad_norm', name = 'T', env='gpu:%d'%args.rank)
                    current_point = utils.ravel_model_params(mod_main, False, 'cpu')
                    l2_norm = (current_point - starting_point).norm().item()
                    result_list += [errors]
                    visdom_obj.line([l2_norm],  [global_epoch + (m_task-1)* args.cl_epochs+cl_epoch], update='append', opts={'title':'L2 Norm'}, win='l2_norm', name = 'T', env='gpu:%d'%args.rank)
                    visdom_obj.line([sum(errors)/args.num_tasks],  [global_epoch + (m_task-1)* args.cl_epochs+cl_epoch], update='append', opts={'title':'Average Error'}, win='avg_error', name = 'T', env='gpu:%d'%args.rank)
                    if adding_new_hidden_layer:
